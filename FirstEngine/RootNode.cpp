...
RootNode::RootNode() : SceneNode(0, nullptr, RenderPass::RenderPass_0) {
	m_Children.reserve(static_cast<int>(RenderPass::RenderPass_Last));

	std::shared_ptr<SceneNode> staticGroup(new SceneNode(0, nullptr, RenderPass::RenderPass_Static));
	m_Children.push_back(staticGroup);	// RenderPass_Static = 0

	std::shared_ptr<SceneNode> actorGroup(new SceneNode(0, nullptr, RenderPass::RenderPass_Actor));
	m_Children.push_back(actorGroup);	// RenderPass_Actor = 1

	std::shared_ptr<SceneNode> skyGroup(new SceneNode(0, nullptr, RenderPass::RenderPass_Sky));
	m_Children.push_back(skyGroup);	// RenderPass_Sky = 2

	std::shared_ptr<SceneNode> invisibleGroup(new SceneNode(0, nullptr, RenderPass::RenderPass_NotRendered));
