...
bool D3DClass::Initialize(std::shared_ptr<EngineOptions> options, HWND hwnd) {
	...
	result = CreateDXGIFactory(IID_PPV_ARGS(m_factory.GetAddressOf()));
	if (FAILED(result)) {
		return false;
	}
	...
	result = m_factory->EnumAdapters(0, adapter.GetAddressOf());
	if (FAILED(result)) {
		return false;
	}
